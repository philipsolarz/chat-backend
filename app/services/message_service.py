...
class MessageService:
    ...
    def create_message(self, 
        ...
        Returns:
            The created message or None if the participant doesn't exist
        """
    ...
    def get_messages(self, 
                    ...
                    after_timestamp: Optional[datetime] = None) -> Tuple[List[Message], int, int]:
        """
        ...
        if filters:
            ...
            if 'character_id' in filters:
                # Filter by character (across all participants using this character)
                query = query.join(
            ...
            if 'user_id' in filters:
                # Filter by user (across all participants for this user)
                query = query.join(
                    ConversationParticipant,
                    Message.participant_id == ConversationParticipant.id
                ).filter(
            ...
            if 'agent_id' in filters:
                # Filter by agent (across all participants for this agent)
                query = query.join(
                    ConversationParticipant,
                    Message.participant_id == ConversationParticipant.id
                ).filter(
    ...
    def get_sender_info(self, message: Message) -> Dict[str, Any]:
        """Get detailed information about the sender of a message"""
        ...
        # Add additional info for user or agent if available
        if participant.user_id:
            user = self.db.query(User).filter(User.id == participant.user_id).first()
            if user:
                result["user_name"] = f"{user.first_name} {user.last_name}".strip() or user.email
    ...
    def search_messages(self, conversation_id: str, query: str, page: int = 1, page_size: int = 20) -> Tuple[List[Message], int, int]:
    ...
    def get_user_message_count(self, user_id: str) -> int:
        """Count the total number of messages sent by a user (across all conversations)"""
